...
export async function POST(req: Request) {
  try {
    ...
    const { logIds, action, notes } = body // action: "APPROVE" | "REJECT"
    ...
    for (const logId of logIds) {
      ...
      if (action === "APPROVE") {
        ...
        await prisma.monitoredProduct.update({
          ...
          data: {
            currentPrice: log.newPrice,
            recommendedAction: "MAINTAIN",
            recommendationReason: `Price successfully updated from $${log.oldPrice.toFixed(2)} to $${log.newPrice.toFixed(2)} upon seller approval.`,
            recommendedPrice: log.newPrice,
          },
        })
      } else {
        // REJECT
